...
5. Update the config/base.json file updates array to include a name and description of the change. The name structure has to be "[{APP_NAME}] {Feature}. New updates should be up the list. Also, keep only the latest 10 versions (but keep all the updates of each version that is kept).
