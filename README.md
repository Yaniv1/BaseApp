...
- **Each task runs on its own branch.** Work on a task now happens on a dedicated short-lived branch named after the task id (e.g. `BASE-TASK-260624-0001`, with no `task/` prefix so the branch name matches its worktree folder exactly) instead of directly on `main`, so concurrent tasks never mix their changes. The branch is created automatically by the task launcher when a worker starts. If a branch already exists for the task, the Task Manager asks you to confirm before starting another worker on it, so in-progress work isn't unknowingly duplicated.
