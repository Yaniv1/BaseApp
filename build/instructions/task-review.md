...
5. If the engineer approves, help finalize per `build/instructions/base.md` (finalize/deploy, set the task to `Done`, commit the task file together with all the changes, and push). If the engineer does not approve, keep the task in `Ready` and capture the requested adjustments as task comments.

Do not change the task to `Done` or commit/push until the engineer explicitly approves.
